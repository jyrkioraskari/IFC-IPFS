...
public class IPFS_ConcurrentFetchDir extends IPFS_Fetch {
	...
	public IPFS_ConcurrentFetchDir(boolean clean, String dir_hash) throws IOException {
	...
	public void fetch(String dir_hash,boolean jena) {
		...
		if (this.clean) {
			...
			this.guid_directory_model.listStatements().toList().stream()
					.filter(s -> s.getPredicate().equals(this.merkle_node)).map(s -> s.getObject()).forEach(x -> {
						Future<Integer> future = taskExecutor1.submit(createreadNodeCallable(model_list.get( getNextJenaModel()),x.asLiteral().getLexicalForm(),jena));
					});
		...
		this.guid_directory_model.listStatements().toList().stream()
				.filter(s -> s.getPredicate().equals(this.merkle_node)).map(s -> s.getObject()).forEach(x -> {
					Future<Integer> future = taskExecutor2.submit(createreadNodeCallable(model_list.get( getNextJenaModel()),x.asLiteral().getLexicalForm(),jena));
				});
	...
	private void readInNode(Model temp_model,String key,boolean jena) {		
		...
		try {
			String tmp1 = new String(ipfs.cat(filePointer));
			long end = System.nanoTime();
			String tmp2 = new String(ipfs.cat(filePointer));
			if(!tmp1.equals(tmp2))
				System.out.println("NOT EQUAL");
			addLog(key+" read  in: " + (end - start)/1000000f + " ms");
			System.out.println("t_"+key+" offset  in: " + (start - total_start)/1000000f + " ms read  in: " + (end - start)/1000000f + " ms");
			ByteArrayInputStream bi = new ByteArrayInputStream(tmp1.getBytes());
			if(jena)
	...
	public static void main(String[] args) {
		...
		try {
			if (args.length == 0)
				new IPFS_ConcurrentFetchDir(true, hash);
			else if (args[0].toLowerCase().trim().equals("true"))
				new IPFS_ConcurrentFetchDir(true, hash);
			else
				new IPFS_ConcurrentFetchDir(false, hash);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
